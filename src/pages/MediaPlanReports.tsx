...
export default function MediaPlanReports() {
  ...
  const handleDeleteImport = async () => {
    ...
    await deleteImport.mutateAsync({
      import_id: importToDelete.id,
      media_plan_id: importToDelete.media_plan_id,
    });
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleReimport(importConfig)}
                        disabled={runImport.isPending}
                      >
                        <RefreshCw className={`w-4 h-4 ${runImport.isPending ? 'animate-spin' : ''}`} />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleEditImport(importConfig)}>
                        <Settings className="w-4 h-4" />
                      </Button>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => {
                          setImportToDelete(importConfig);
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja remover esta fonte de dados? Todos os dados importados serão
